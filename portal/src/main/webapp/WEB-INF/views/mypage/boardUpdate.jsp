...
<head>
    ...
    function fn_update() {
        ...
        $.ajax({
            url : "${httpCtxPath}/mypage/boardUpdateProcess.action",
            type : "POST",
            dataType : "json",
            data : params,
            async : false,
            success : function(response, status, request) {
                if(response == "1") {
                    alert("정상적으로 ${boardTitle}이 수정되었습니다.");
                    fn_list();
                } else {
                    alert("${boardTitle} 수정처리 중 오류가 발생하였습니다.\n관리자에게 문의하시기 바랍니다.");
                }
            },
    ...
    function fn_delete() {
        if(!confirm("정말로 ${boardTitle}을 삭제하시겠습니까?")) return;
        var params = $("form[name=insertForm]").serialize();

        $.ajax({
            url : "${httpCtxPath}/mypage/boardDeleteProcess.action",
            type : "POST",
            dataType : "json",
            data : params,
            async : false,
            success : function(response, status, request) {
                if(response == "1") {
                    alert("정상적으로 ${boardTitle}이 삭제되었습니다.");
                    fn_list();
                } else {
                    alert("${boardTitle} 삭제처리 중 오류가 발생하였습니다.\n관리자에게 문의하시기 바랍니다.");
                }
            },
            error : function(request,status,error) {
                alert("시스템 오류가 발생하였습니다.\n관리자에게 문의하시기 바랍니다.");
    ...
    <div class="my_page">
        ...
        <div class="my_contents">
            <div class="content_title">
                <h2 style="float:left;">${boardTitle} 상세</h2>
                <p style="float:right;">HOME &gt; 마이페이지 &gt; ${boardTitle}관리 &gt; ${boardTitle} 상세</p>
            </div>
            ...
            <div class="room_bottom_wrap">
                ...
                <tr>
                    ...
                    <td style="text-align:left;padding-left:10px"><input id="subject" name="subject" type="text" maxlength="100" size="100" value="${boardInfo.subject}"></td>
                </tr>
                <c:if test="${params.board_gubun eq 'E' }">
                <tr>
                    ...
                    <td style="text-align:left;padding-left:10px">
                        <input id="event_start_date" name="event_start_date" value="${boardInfo.event_start_date}" type="text" maxlength="10" size="10" readonly="readonly">
                         ~ 
                         <input id="event_end_date" name="event_end_date" value="${boardInfo.event_end_date}"  type="text" maxlength="10" size="10" readonly="readonly">
                    </td>
                </tr>
                </c:if>
                <tr>
                    ...
                    <td style="text-align:left;padding-left:10px;padding-right:10px"><textarea id="content" name="content" style="width:100%; height:300px; display:none;">${boardInfo.content}</textarea></td>
                </tr>
                </tbody>
                </table>
                <div class="bt_group">
                    <p class="text_bt_style"><a href="javascript:fn_list();" target="_self" style="background:#333; margin-left:10px;">목록</a></p>
                    <p class="text_bt_style"><a href="javascript:fn_delete();" target="_self" style="margin-left:10px;">삭제</a></p>
                    <p class="text_bt_style"><a href="javascript:fn_update();" target="_self">수정</a></p>
                </div>
